import config
from utils import (set_random_seed, NumericalEstimateEncoder, CategoricalEstimateEncoder, NumericalStdDevEstimateEncoder, get_scheduler)
...
def main():
    ...
    for experiment_number in range(config.EXPERIMENT_REPEAT):
        ...
        for col in NUM_COLS:
            encoder = NumericalStdDevEstimateEncoder(feature_col=col, target_col='label', m=len(train_data)*config.ENCODING_SMOOTHING)
