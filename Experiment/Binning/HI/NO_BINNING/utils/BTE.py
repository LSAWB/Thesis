...
class NumericalStdDevEstimateEncoder:
    def __init__(self, feature_col, target_col, m=10):
        self.feature_col = feature_col
        self.target_col = target_col
        self.m = m  # 平滑強度
    ...
    def fit(self, df: pd.DataFrame):
        ...
        self.global_mean = self.train_y.mean()
    ...
    def transform(self, df: pd.DataFrame) -> pd.Series:
        ...
        for x_i in x_values:
            ...
            if len(neighbor_y) > 0:
                mu = neighbor_y.mean()
                n = len(neighbor_y)
                smoothed = (n * mu + self.m * self.global_mean) / (n + self.m)
            else:
                smoothed = self.global_mean  # 沒有鄰居時 fallback
