...
class NumericalStdDevEstimateEncoderMultiClass:
    def __init__(self, feature_col, target_col, m=10):
        self.feature_col = feature_col
        self.target_col = target_col
    ...
    def fit(self, df: pd.DataFrame):
        self.train_x = df[self.feature_col].values
        self.train_y = df[self.target_col].values
        self.std = self.train_x.std()

        self.class_labels = np.unique(self.train_y)
        num_classes = len(self.class_labels)
        class_counts = np.bincount(self.train_y, minlength=num_classes)
        self.global_class_probs = class_counts / class_counts.sum()

    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        x_values = df[self.feature_col].values
        result = []

        for x_i in x_values:
            # Find neighbors within ±std
            mask = np.abs(self.train_x - x_i) <= self.std
            neighbor_y = self.train_y[mask]

            if len(neighbor_y) > 0:
                counts = np.bincount(neighbor_y, minlength=len(self.class_labels))
                smoothed_probs = (counts + self.m * self.global_class_probs) / (len(neighbor_y) + self.m)
            else:
                smoothed_probs = self.global_class_probs

            result.append(smoothed_probs)
