...
def main():
    ...
    for experiment_number in range(experiment_repeat):
        ...
        model = FTTransformer(
            ...
            n_blocks=2,
            d_block=232,
            ffn_d_hidden_multiplier=1.769397554242595,
            residual_dropout=0.011435047063502825,
            attention_dropout=0.008028708593695255,
            ffn_dropout=0.19800429837185157,
            linformer_kv_compression_ratio=0.2,
            linformer_kv_compression_sharing='headwise',
        ).to(device)
        criterion = torch.nn.MSELoss()

        learning_rate = 0.00041399803858633064
        weight_decay = 8.346664037161856e-05
