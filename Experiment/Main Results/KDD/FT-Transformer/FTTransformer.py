...
def main():
    ...
    for experiment_number in range(experiment_repeat):
        ...
        model = FTTransformer(
            ...
            n_blocks=2,
            d_block=144,
            ffn_d_hidden_multiplier=1.6464058750393336,
            residual_dropout=0.12034953902901357,
            attention_dropout=0.27379413070773645,
            ffn_dropout=0.3559626085471593,
            linformer_kv_compression_ratio=0.2,
            linformer_kv_compression_sharing='headwise',
        ).to(device)

        criterion  = F.cross_entropy
        learning_rate = 0.00010907571752305717
        weight_decay = 2.2454860621973057e-06
