...
def main():
    ...
    for experiment_number in range(config.EXPERIMENT_REPEAT):
        ...
        model = DualTransformer(
            ...
            dim_ff=int(296 * 1.3107624867949275),
            num_layers_cross=1,
            num_labels=config.NUM_LABELS,
            return_attention=True,
            att_dropout=0.01234097712782628,
            res_dropout=0.13360660817105666,
            ffn_dropout=0.3600985629099332
        ).to(config.DEVICE)

        # Optimizer & Scheduler
        optimizer = torch.optim.AdamW(model.parameters(), lr=0.00038998473506302435, weight_decay=0.0004816266095178276)
