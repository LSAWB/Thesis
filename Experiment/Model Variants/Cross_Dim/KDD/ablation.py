...
def main():
    ...
    for experiment_number in range(config.EXPERIMENT_REPEAT):
        ...
        model = DualTransformer(
            ...
            dim_ff=int(176 * 2.2937689120170766),
            num_layers_cross=5,
            num_labels=config.NUM_LABELS,
            return_attention=True,
            att_dropout=0.32235118277806063,
            res_dropout=0.1103114996190381,
            ffn_dropout=0.24717405783446966
        ).to(config.DEVICE)

        # Optimizer & Scheduler
        optimizer = torch.optim.AdamW(model.parameters(), lr=5.249738114152329e-05, weight_decay=0.0008499003844891884)
