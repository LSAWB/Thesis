import config
from utils import (set_random_seed, NumericalEstimateEncoder, NumericalEstimateEncoderMultiClass, CategoricalEstimateEncoder, get_scheduler)
...
def main():
    ...
    for experiment_number in range(config.EXPERIMENT_REPEAT):
        ...
        for col in NUM_COLS:
            encoder = NumericalEstimateEncoderMultiClass(feature_col=col, target_col='label', bin_size=config.ENCODING_BINSIZE, m=len(train_data)*config.ENCODING_SMOOTHING)
