...
class BTEEmbeddings(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 3:
            raise ValueError(f'Expected shape (B, N, D), got {x.shape}')
        x = x.transpose(0, 1)  # (N, B, D)
        x = torch.matmul(x, self.weight)  # (N, B, E)
...
class NormalAttentionLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff=None, attention_dropout=0.1, residual_dropout=0.1, ffn_dropout=0.1, return_attention=True):
        ...
        self.attention = AttentionLayer(d_model, n_heads, attention_dropout=attention_dropout, return_attention=return_attention)
    ...
    def forward(self, x):
        ...
        dim_in_norm = self.norm1(x)

        dim_enc, dim_attn = self.attention(dim_in_norm, dim_in_norm, dim_in_norm)
        dim_out = dim_in_norm + self.residual_dropout(dim_enc)
        dim_ffn = dim_out + self.ffn_dropout(self.MLP1(self.norm2(dim_out)))

        if self.return_attention:
            return dim_ffn, None, None
        else:
            return dim_ffn
...
#         self.input_token = nn.Parameter(torch.randn(1, 1, 2, d_model)) 
        
#     def forward(self, x):
#         batch, dim_num, sta_num, d_model = x.shape
#         sta_attentions, dim_attentions = [], []
        
#         global_token = self.input_token.expand(batch, -1, -1, -1)
#         x = torch.cat([global_token, x], dim=1)
        
#         for i, layer in enumerate(self.layers):
#             x, s_attn, d_attn = layer(x)
#             # dim_token = self.cross(dim_token, sta_token)
...
class DualTransformer(nn.Module):
    def __init__(self, 
        ...
        self.bte_embedding_layer        = BTEEmbeddings(n=statistic_features, in_features=num_labels, out_features=dim_model)
        ...
        # === Classifier ===
        self.classifier = nn.Linear(dim_model, num_labels)
    ...
    def forward(self, cat_data, num_data, bte_data):
        ...
        # === CAT ===
        if  cat_len > 0:
            cat_emb = self.cat_ori_embedding_layer(cat_data)
        else:
            cat_emb = torch.empty(batch_size, 0, self.num_ori_embedding_layer.d_embedding, device=bte_data.device)
                     
        # === NUM ===
        if num_len > 0:
            num_emb = self.num_ori_embedding_layer(num_data)
        else:
            num_emb = torch.empty(batch_size, 0, self.num_ori_embedding_layer.d_embedding, device=bte_data.device)

        # === BTE ===
        bte_input = bte_data.reshape(batch_size, ori_seq_len, -1)
        bte_embedded    = self.bte_embedding_layer(bte_input)
        
        # === Combine ===
        tmp_embedded = torch.cat([cat_emb, num_emb], dim=1)
        all_embedded = torch.cat([tmp_embedded, bte_embedded], dim=1)
        all_with_pos = all_embedded

        # === CLS token + Positional Encoding ===
        cls_tok = self.cls_token.expand(batch_size, -1, -1)  # [B, 1, d_model]
        all_emb = torch.cat([cls_tok, all_with_pos], dim=1)  # [B, L+1, d_model]
        
        # === Attention ===
        final_out, _, _= self.TwoStage(all_emb)
