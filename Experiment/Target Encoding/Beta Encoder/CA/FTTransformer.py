...
def main():
    ...
    for experiment_number in range(experiment_repeat):
        ...
        for col in NUM_COLS:
            encoder = NumericalBetaEncoder(feature_col=col, target_col='label', bin_size=0.001, N_min=len(train_data)*0.15)
        ...
        for col in CAT_COLS:
            encoder = CategoricalBetaEncoder(feature_col=col, target_col='label', N_min=len(train_data)*0.15)
            encoder.fit(train_data)
