...
def main():
    ...
    for experiment_number in range(experiment_repeat):
        ...
        for col in NUM_COLS:
            encoder = NumericalEstimateEncoder(feature_col=col, target_col='label', bin_size=0.001, m=len(train_data)*0.05)
        ...
        for col in CAT_COLS:
            encoder = CategoricalEstimateEncoder(feature_col=col, target_col='label', m=len(train_data)*0.05)
            encoder.fit(train_data)
        ...
        val_dataset     = CustomDataset(X_valid_bte_scaled, None, y_val)
