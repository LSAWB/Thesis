...
class NumericalShrinkEncoderMultiClass:
    ...
    def fit(self, df):
        ...
        # Step 1: Normalize + Bin
        x_scaled = self.scaler.fit_transform(x).flatten()
        bin_indices = (x_scaled / self.bin_size).astype(int)

        # Step 2: Assign y to bins
        for i, bin_idx in enumerate(bin_indices):
            self.bin_to_y[bin_idx].append(y[i])

        self.std_bin = int(np.std(bin_indices))

        # Step 3: Global class probabilities
        class_counts = np.bincount(y, minlength=num_classes)
        self.global_probs = class_counts / class_counts.sum()

        # Step 4: Compute smoothed class probabilities per bin
        for bin_idx in self.bin_to_y:
            ...
            n_b = len(neighbor_y)
            p_b = np.bincount(neighbor_y, minlength=num_classes) / n_b

            B_k = 1 / (1 + np.exp(-(n_b - self.S1) / self.S2))
            phi = B_k * p_b + (1 - B_k) * self.global_probs
    ...
    def transform(self, df):
        ...
        for bin_idx in bin_indices:
            if bin_idx in self.bin_to_phi:
                ...
                result.append(weighted_sum / total_weight if total_weight > 0 else self.global_probs)
...
def main():
    ...
    for experiment_number in range(experiment_repeat):
        ...
        for col in NUM_COLS:
            encoder = NumericalShrinkEncoderMultiClass(feature_col=col, target_col='label', bin_size=0.001)
        ...
        model = FTTransformer(
            n_cont_features=len(NUM_COLS) * n_classes,
            cat_cardinalities=[],
            d_out=n_classes,
